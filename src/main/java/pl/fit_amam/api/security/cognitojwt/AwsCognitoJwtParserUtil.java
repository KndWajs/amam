...
public class AwsCognitoJwtParserUtil {
    ...
    public static String getJsonWebKeyURL(String token) throws TokenException {
        ...
        if (Objects.isNull(issJsonElement)) {
            throw  new TokenException(HttpStatus.UNAUTHORIZED, AuthenticationError.NOT_VALID_JSON_WEB_TOKEN, payload.toString());
        }
    ...
    public static File getJsonWebKeyFile(String token) throws TokenException, IOException{
        ...
        if (APIRestUtil.isHTTPError(responseEntity.getStatusCode())) {
            throw new TokenException(responseEntity.getStatusCode(), AuthenticationError.NOT_VALID_JSON_WEB_TOKEN, responseEntity.getBody());
